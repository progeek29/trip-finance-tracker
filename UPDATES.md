...
- **Siren everywhere**: single tap bhi broadcast (`bell` type msg — receivers pe soft chime + flash) · emergency rings App-root overlay pe (landing/dashboard/profile, har screen) + sender heartbeat se persistent · dynamic host (phone pe LAN IP) taaki cross-device socket/REST chale.
